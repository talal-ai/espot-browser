...
async function syncElectronAssets() {
  ...
  // Copy static shell HTML used by service windows (URL bar + webview)
  const shellSrc  = path.resolve('electron', 'service-shell.html');
  const shellDest = path.resolve('dist-electron', 'service-shell.html');
  await cp(shellSrc, shellDest, { force: true });
  console.log('🧱 Copied service-shell.html into dist-electron/');
}
